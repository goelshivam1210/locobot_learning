...
class ArmServer:
    ...
    def handle_grasp_object(self, request):
        ...
        x, y, z = self.cartesian_coords.x, self.cartesian_coords.y, self.cartesian_coords.z

        # Move the arm to the specified pose
        self.pose = PoseStamped()
        self.pose.header.stamp = rospy.Time.now()
        self.pose.header.frame_id = "locobot/arm_base_link"
        self.pose.pose.position = Point(x, y, z)
        self.pose.pose.orientation = Quaternion(0.0, 0.0, 0.0, 1.0)  # Set orientation as needed

        flag = False
        if request.target_object == 'novel_object':
            flag = True 
        # Move the arm to the specified pose
        self.move_arm(self.pose, flag)
